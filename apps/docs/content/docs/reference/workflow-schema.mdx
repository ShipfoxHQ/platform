...
=======
expressions, resolved at step dispatch. A session is scoped to one run attempt,
and a rerun starts it fresh. It is pinned to the harness that created it. For
an existing session, a step that omits `harness` inherits the pinned harness,
whether it resumes or forks the session.
>>>>>>> 795efb4f2 (Implement issue)
