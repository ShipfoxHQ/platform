...
INSERT INTO "definitions_workflows" ("id", "project_id", "config_path")
SELECT "id", "project_id", "id"::text
FROM "definitions_workflow_definitions"
WHERE "config_path" IS NULL;--> statement-breakpoint
UPDATE "definitions_workflow_definitions" AS "d"
SET "workflow_id" = COALESCE(
	(SELECT "w"."id" FROM "definitions_workflows" AS "w"
	 WHERE "w"."project_id" = "d"."project_id" AND "w"."config_path" = "d"."config_path"),
	"d"."id"
);--> statement-breakpoint
